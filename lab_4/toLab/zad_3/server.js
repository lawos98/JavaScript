...
function requestListener(request, response) {
  ...
  if (url.pathname == '/submit') {
    script.params.f(url.searchParams.get('name'), (err, data) => {
    ...
    response.write(`<form method="GET" action="/submit">
	    					<label for="name">Give a path</label>
	    					<input name="name">
	    					<br>
	    					<input type="submit">
	    					<input type="reset">
...
// Sprawdź zachowanie się skryptu dla następujących adresów:
//     http://localhost:8080/submit?name=Jan            --> analogicznie do poprzedniego przykładu – wyświetla się: 'Hello Jan'
//     http://localhost:8080/submit?nazwisko=Kowalski   --> wyświetla się 'Hello null'
//     http://localhost:8080/dowolnyTekst               --> wyświetla się ta sama strona co sam localhost:8080/
